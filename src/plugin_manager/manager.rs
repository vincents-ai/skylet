...
use super::rpc_global::get_global_rpc_registry;
...
impl PluginManager {
    ...
    pub async fn load_plugin_instance_v2(&self, name: &str, path: &PathBuf) -> Result<()> {
        ...
        // Register this plugin's RPC registry with the global registry for cross-plugin calls
        // The per-plugin RPC registry is stored in resources.services_ptr
        let global_registry = get_global_rpc_registry();
        unsafe {
            let services = &*(resources.services_ptr as *const PluginServices);
    ...
    /// Re-initialize a bootstrap plugin with the full PluginContextV2
    ///
    /// Bootstrap plugins are loaded during startup with a minimal context (null service registry).
    /// This method re-initializes them with the full context so their services are properly
    /// registered in the service registry.
    pub async fn init_bootstrap_plugin(&self, name: &str, library: &Library) -> Result<()> {
        // Extract the init function from the already-loaded library
        let init_fn = unsafe {
            library
        ...
        // Call the plugin's init function with the full context
        let result = init_fn(&context_v2 as *const PluginContextV2);
        if result != PluginResultV2::Success {
            return Err(anyhow!(
                "Bootstrap plugin '{}' init failed with result: {:?}",
                name,
                result
            ));
        }
        ...
        let mut resources_map = self.plugin_resources.write().await;
        resources_map.insert(format!("bootstrap:{}", name), resources);
    ...
    fn create_plugin_context_v2(
        ...
        let per_plugin_services = Arc::new(PluginServices {
            ...
            rpc_registry: per_plugin_rpc_registry,
            tracer: self.services.tracer.clone(),
            secrets: self.services.secrets.clone(),
            http_router: self.services.http_router.clone(),
        });
        ...
        let services_ptr = Arc::into_raw(per_plugin_services);
    ...
    extern "C" fn rpc_v2_register_handler(
        ...
        // Create a wrapper that bridges the C FFI handler to the Rust RpcHandler type
        let handler_wrapper = Arc::new(move |request: &[u8]| {
            // Convert request bytes to C string for RpcRequestV2
            let request_cstring = match CString::new(request) {
                Ok(s) => s,
                Err(_) => return (PluginResultV2::InvalidRequest, vec![]),
            };

            let rpc_request = RpcRequestV2 {
                method: std::ptr::null(),
                params: request_cstring.as_ptr(),
                timeout_ms: 30000,
            };

            // Prepare response struct
            let mut rpc_response = RpcResponseV2 {
                result: std::ptr::null(),
                error: std::ptr::null(),
            ...
            // Call the C handler
            handler(&rpc_request, &mut rpc_response);

            let status = rpc_response.status;

            // Extract result bytes from response
            let result_bytes = if !rpc_response.result.is_null() {
                let cstr = unsafe { CStr::from_ptr(rpc_response.result) };
