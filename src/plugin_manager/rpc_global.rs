...
impl GlobalRpcRegistry {
    ...
    pub fn register_plugin(&self, plugin_name: &str, registry: Arc<RpcRegistry>) {
        ...
        eprintln!("[rpc_global] Registering plugin '{}' with RPC registry", plugin_name);
        registries.push((plugin_name.to_string(), registry));
        eprintln!("[rpc_global] Total registered plugins: {}", registries.len());
    }
    ...
    pub fn call(&self, service: &str, params: &[u8]) -> Result<Vec<u8>, PluginResultV2> {
        let registries_clone: Vec<(String, Arc<RpcRegistry>)> = {
            ...
            eprintln!("[rpc_global] Looking for service '{}' across {} registries", service, registries.len());
            
            for (plugin_name, registry) in registries.iter() {
                let services = registry.list_services();
                eprintln!("[rpc_global] Plugin '{}' has services: {:?}", plugin_name, services);
            }
        ...
        for (plugin_name, registry) in registries_clone.iter() {
            ...
            match registry.call(service, params) {
                Ok(bytes) => {
                    eprintln!("[rpc_global] Found service '{}' in plugin '{}'", service, plugin_name);
                    return Ok(bytes);
                }
                Err(PluginResultV2::ServiceUnavailable) => {
                    eprintln!("[rpc_global] Service '{}' not found in plugin '{}', trying next", service, plugin_name);
                    continue;
                }
                Err(e) => {
        ...
        eprintln!("[rpc_global] Service '{}' not found in any registry", service);
        Err(PluginResultV2::ServiceUnavailable)
    }
}
