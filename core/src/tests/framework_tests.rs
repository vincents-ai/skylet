...
#[cfg(test)]
mod framework_tests {
    use crate::framework::{TestFramework, TestConfiguration};
    ...
    fn test_framework_configuration() {
        ...
        assert!(config.cleanup_on_drop, "Default config should cleanup on drop");
        assert_eq!(config.max_test_duration_secs, 300, "Default max duration should be 300");
    }
