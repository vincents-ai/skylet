...
mod integration_tests {
    ...
    fn test_service_compatibility() {
        ...
        let _env1 = framework
            .create_test_environment("parallel_test_1")
            .unwrap();
        let _env2 = framework
            .create_test_environment("parallel_test_2")
            .unwrap();
        ...
        assert!(
            framework.get_environment("parallel_test_1").is_none(),
            "Environment 1 should be cleaned up"
        );
        assert!(
            framework.get_environment("parallel_test_2").is_none(),
            "Environment 2 should be cleaned up"
        );
    ...
    fn test_performance_under_load() {
        ...
        assert!(
            ...
            "Environment creation should complete within 30 seconds"
        );

        let _ = framework.cleanup_all();

        tracing::info!(
            "✅ Performance under load test passed (creation: {:?}, total: {:?})",
            creation_time,
            total_time
        );
    }
