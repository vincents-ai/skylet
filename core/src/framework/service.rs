...
impl MockServiceRegistry {
    ...
    pub fn register_service(&mut self, name: &str, type_name: &str, ptr: *mut std::ffi::c_void) {
        self.services.push((name.to_string(), type_name.to_string(), ptr));
    }
...
mod tests {
    ...
    fn test_lifecycle_simulator() {
        ...
        simulator.emit_event(LifecycleEvent::PluginLoaded { name: "test".to_string() });
        simulator.emit_event(LifecycleEvent::PluginInitialized { name: "test".to_string() });
