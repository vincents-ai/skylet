...
impl AuthorizationMiddleware {
    ...
    pub fn authorize(&self, context: &UserContext, required_permission: &Permission) -> Result<(), AuthzError> {
        if self.checker.has_permission(&context.user_id, required_permission) {
            Ok(())
        } else {
    ...
    pub fn authorize_any(&self, context: &UserContext, permissions: &[Permission]) -> Result<(), AuthzError> {
        if self.checker.has_any_permission(&context.user_id, permissions) {
            Ok(())
        } else {
            Err(AuthzError::PermissionDenied {
                user_id: context.user_id.0.clone(),
                permission: permissions.iter()
                    .map(|p| p.as_str())
                    .collect::<Vec<_>>()
                    .join(", "),
            })
...
impl ResourcePermission {
    pub fn new(resource_type: impl Into<String>, resource_id: impl Into<String>, action: impl Into<String>) -> Self {
        Self {
