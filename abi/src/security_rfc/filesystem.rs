...
use super::capabilities::{FilesystemAccessMode, CapabilityStatus};
...
impl FilesystemEnforcer {
    ...
    fn mode_compatible(&self, granted: FilesystemAccessMode, requested: FilesystemAccessMode) -> bool {
...
impl fmt::Display for FilesystemAccessError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FilesystemAccessError::PermissionDenied { plugin_id, path, mode } => {
                write!(f, "Permission denied for plugin {} to {:?} {}", plugin_id, mode, path)
            }
...
mod tests {
    ...
    fn test_mode_compatible() {
        ...
        assert!(enforcer.mode_compatible(FilesystemAccessMode::ReadWrite, FilesystemAccessMode::Read));
        assert!(enforcer.mode_compatible(FilesystemAccessMode::ReadWrite, FilesystemAccessMode::ReadWrite));
        assert!(enforcer.mode_compatible(FilesystemAccessMode::ReadWrite, FilesystemAccessMode::WriteOnly));
        
        // Read only grants read
        assert!(enforcer.mode_compatible(FilesystemAccessMode::Read, FilesystemAccessMode::Read));
        assert!(!enforcer.mode_compatible(FilesystemAccessMode::Read, FilesystemAccessMode::WriteOnly));
    }
    ...
    fn test_check_access_granted() {
        ...
        enforcer.add_permission("test-plugin", PathPermission {
        ...
        assert!(enforcer.check_access("test-plugin", "plugin-data://cache/file.txt", FilesystemAccessMode::Read).is_ok());
        assert!(enforcer.check_access("test-plugin", "plugin-data://cache/file.txt", FilesystemAccessMode::WriteOnly).is_ok());
    }
    ...
    fn test_check_access_denied() {
        ...
        let result = enforcer.check_access("unknown-plugin", "/etc/shadow", FilesystemAccessMode::Read);
